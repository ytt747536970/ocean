...
Texture2D g_texCurrStep : register(t1);
...
{ 
	...
	// cos(pi * (m1 + m2))
	int sign_correction = ((index_x + index_y) & 1) ? -1 : 1;            //相当于pow(-1,n) 如果相加为偶数，则乘1，否则乘-1
	...
	float dz = g_InputDxyz[addr].x * sign_correction;                    //这应该是高度
...
float4 GenGradientFoldingPS(VS_QUAD_OUTPUT In) : SV_Target
{
	...
	float3 displace_left  = g_samplerDisplacementMap.Sample(LinearSampler, tc_left).xyz + g_texCurrStep.Sample(LinearSampler, tc_left);
	float3 displace_right = g_samplerDisplacementMap.Sample(LinearSampler, tc_right).xyz + g_texCurrStep.Sample(LinearSampler, tc_right);
	float3 displace_back  = g_samplerDisplacementMap.Sample(LinearSampler, tc_back).xyz + g_texCurrStep.Sample(LinearSampler, tc_back);
	float3 displace_front = g_samplerDisplacementMap.Sample(LinearSampler, tc_front).xyz + g_texCurrStep.Sample(LinearSampler, tc_front);
