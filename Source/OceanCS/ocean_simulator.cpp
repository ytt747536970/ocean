...
void OceanSimulator::Change(OceanParameter& params)
{
	// Height map H(0)
...
OceanSimulator::OceanSimulator(OceanParameter& params, ID3D11Device* pd3dDevice)
{
	...
	createTextureAndViews(m_pd3dDevice, hmap_dim, hmap_dim, DXGI_FORMAT_R16G16B16A16_FLOAT, &m_pCurrStepMap, &m_pCurrStepMapSRV, &m_pCurrStepMapRTV);
	// Samplers
...
void OceanSimulator::updateDisplacementMap(float time,ID3D11ShaderResourceView* gCurrStepSRV)
{
	// ---------------------------- H(0) -> H(t), D(x, t), D(y, t) --------------------------------
	// Compute shader
	...
	// VS & PS
	m_pd3dImmediateContext->VSSetShader(m_pQuadVS, NULL, 0);                            //"ocean_simulator_vs_ps.hlsl"жа"QuadVS"
	m_pd3dImmediateContext->PSSetShader(m_pUpdateDisplacementPS, NULL, 0);              //"ocean_simulator_vs_ps.hlsl"жа"UpdateDisplacementPS"
	...
	m_pd3dImmediateContext->PSSetShader(m_pGenGradientFoldingPS, NULL, 0);   //"ocean_simulator_vs_ps.hlsl"жа"GenGradientFoldingPS"

	// Texture resource and sampler
	ps_srvs[0] = m_pDisplacementSRV;
	ps_srvs[1] = gCurrStepSRV;
    m_pd3dImmediateContext->PSSetShaderResources(0, 2, ps_srvs);
...
ID3D11ShaderResourceView* OceanSimulator::getD3D11CurrStepMap()
{
	return m_pCurrStepMapSRV;
}
