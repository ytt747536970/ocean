...
HRESULT DXUTFindValidD3D11DeviceSettings( DXUTD3D11DeviceSettings* pOut, DXUTD3D11DeviceSettings* pIn, DXUTMatchOptions* pMatchOptions, DXUTD3D11DeviceSettings* pOptimal );
void    DXUTBuildOptimalD3D11DeviceSettings( DXUTD3D11DeviceSettings* pOptimalDeviceSettings, DXUTD3D11DeviceSettings* pDeviceSettingsIn, DXUTMatchOptions* pMatchOptions );
bool    DXUTDoesD3D11DeviceComboMatchPreserveOptions( CD3D11EnumDeviceSettingsCombo* pDeviceSettingsCombo, DXUTD3D11DeviceSettings* pDeviceSettingsIn, DXUTMatchOptions* pMatchOptions );
float   DXUTRankD3D11DeviceCombo( CD3D11EnumDeviceSettingsCombo* pDeviceSettingsCombo, DXUTD3D11DeviceSettings* pOptimalDeviceSettings, DXGI_MODE_DESC* pAdapterDisplayMode );
void    DXUTBuildValidD3D11DeviceSettings( DXUTD3D11DeviceSettings* pValidDeviceSettings, CD3D11EnumDeviceSettingsCombo* pBestDeviceSettingsCombo, DXUTD3D11DeviceSettings* pDeviceSettingsIn, DXUTMatchOptions* pMatchOptions );
HRESULT DXUTFindValidD3D11Resolution( CD3D11EnumDeviceSettingsCombo* pBestDeviceSettingsCombo, DXGI_MODE_DESC displayModeIn, DXGI_MODE_DESC* pBestDisplayMode );
...
HRESULT WINAPI DXUTFindValidDeviceSettings( DXUTDeviceSettings* pOut, DXUTDeviceSettings* pIn, DXUTMatchOptions* pMatchOptions )
{
    ...
    if( !bAppSupportsD3D9 && !bAppSupportsD3D10 && !bAppSupportsD3D11 )
    {
        bAppSupportsD3D9 = true;
        bAppSupportsD3D10 = true;
        bAppSupportsD3D11 = true;
    }

    bool bAvoidD3D9 = false;
    bool bAvoidD3D10 = false;
    bool bAvoidD3D11 = false;
    if( pMatchOptions->eAPIVersion == DXUTMT_PRESERVE_INPUT && pIn && pIn->ver == DXUT_D3D11_DEVICE )
    {
        bAvoidD3D9 = true;
        bAvoidD3D10 = true;
    }
    ...
    if( bAppSupportsD3D11 && !bPreferD3D9 && !bAvoidD3D11 )
    {
        bool bD3D11Available = DXUTIsD3D11Available();
        if( bD3D11Available )
        {
            // Force an enumeration with the IsDeviceAcceptable callback
            DXUTGetD3D11Enumeration( false );

            DXUTD3D11DeviceSettings d3d11In;
            ZeroMemory( &d3d11In, sizeof( DXUTD3D11DeviceSettings ) );
            if( pIn )
            {
                if( pIn->ver == DXUT_D3D9_DEVICE )
                    DXUTConvertDeviceSettings9to11( &pIn->d3d9, &d3d11In );
                else if( pIn->ver == DXUT_D3D10_DEVICE )
                    DXUTConvertDeviceSettings10to11( &pIn->d3d10, &d3d11In );
                else
                    d3d11In = pIn->d3d11;
            }

            optimalDeviceSettings.ver = DXUT_D3D11_DEVICE;
            DXUTBuildOptimalD3D11DeviceSettings( &optimalDeviceSettings.d3d11, &d3d11In, pMatchOptions );

            validDeviceSettings.ver = DXUT_D3D11_DEVICE;
            hr = DXUTFindValidD3D11DeviceSettings( &validDeviceSettings.d3d11, &d3d11In,
                                                   pMatchOptions, &optimalDeviceSettings.d3d11 );
            if( SUCCEEDED( hr ) )
                bFoundValidD3D11 = true;
        }
        else
        {
            if( bAvoidD3D9 )
    ...
    if( bAppSupportsD3D10 && !bFoundValidD3D11 && !bPreferD3D9 && !bAvoidD3D10 )
    {
    ...
    if( bAppSupportsD3D9 && !bFoundValidD3D11 && !bFoundValidD3D10 && !bAvoidD3D9 )
    {
        // Force an enumeration with the IsDeviceAcceptable callback
    ...
    if( bFoundValidD3D11 || bFoundValidD3D10 || bFoundValidD3D9 )
    {
...
HRESULT CD3D10Enumeration::EnumerateDevices( CD3D10EnumAdapterInfo* pAdapterInfo )
{
    ...
    for( UINT iDeviceType = 0; iDeviceType < devTypeArrayCount; iDeviceType++ )
    {
        ...
        hr = DXUT_Dynamic_D3D10CreateDevice( pAdapter, devTypeArray[iDeviceType], ( HMODULE )0, 0, NULL,
                                          ...
                                          DXUTMatchOptions* pMatchOptions )
{
    ...
    {
        640, 480, { 60, 1 }, DXGI_FORMAT_R8G8B8A8_UNORM_SRGB
    };
    ...
    if( pMatchOptions->eRefreshRate == DXUTMT_PRESERVE_INPUT )
    {
        ...
        for( int i = 0; i < pDeviceSettingsCombo->pOutputInfo->displayModeList.GetSize(); i++ )
        {
            ...
            if( fabs( float( displayMode.RefreshRate.Numerator ) / displayMode.RefreshRate.Denominator -
                      float( pDeviceSettingsIn->sd.BufferDesc.RefreshRate.Numerator ) /
                      pDeviceSettingsIn->sd.BufferDesc.RefreshRate.Denominator ) < 0.1f )
            {
                bFound = true;
                break;
            }
        ...
        else
        {
            ...
            //            if( refreshRateMatch != 0 )
            {
                ...
                for( int iDisplayMode = 0; iDisplayMode < pDisplayModeList->GetSize(); iDisplayMode++ )
                {
                    ...
                    // Find the delta between the current refresh rate and the optimal refresh rate
                    float fCurRanking = abs( float( displayMode.RefreshRate.Numerator ) /
...
//--------------------------------------------------------------------------------------
CD3D11Enumeration* WINAPI DXUTGetD3D11Enumeration( bool bForceEnumerate, bool bEnumerateAllAdapterFormats )
{
    // Using an static class with accessor function to allow control of the construction order
    ...
    {
        g_pDXUTD3D11Enumeration->SetEnumerateAllAdapterFormats( bEnumerateAllAdapterFormats, false );
...
//--------------------------------------------------------------------------------------
HRESULT CD3D11Enumeration::EnumerateDeviceCombos( IDXGIFactory* pFactory, CD3D11EnumAdapterInfo* pAdapterInfo )
{
    // Iterate through each combination of device driver type, output,
    // adapter format, and backbuffer format to build the adapter's device combo list.
    //
...
//--------------------------------------------------------------------------------------
void CD3D11Enumeration::SetEnumerateAllAdapterFormats( bool bEnumerateAllAdapterFormats, bool bEnumerateNow )
{
    m_bEnumerateAllAdapterFormats = bEnumerateAllAdapterFormats;
...
void CD3D11Enumeration::BuildMultiSampleQualityList( DXGI_FORMAT fmt, CD3D11EnumDeviceSettingsCombo* pDeviceCombo )
{
    ...
    if( FAILED( DXUT_Dynamic_D3D11CreateDevice( pAdapter,
...
//--------------------------------------------------------------------------------------
HRESULT DXUTFindValidD3D11DeviceSettings( DXUTD3D11DeviceSettings* pOut, DXUTD3D11DeviceSettings* pIn,
                                          DXUTMatchOptions* pMatchOptions, DXUTD3D11DeviceSettings* pOptimal )
{
    // Find the best combination of:
    //      Adapter Ordinal
    //      Device Type
    ...
    //      Windowed
    // given what's available on the system and the match options combined with the device settings input.
    // This combination of settings is encapsulated by the CD3D11EnumDeviceSettingsCombo class.
    float fBestRanking = -1.0f;
    CD3D11EnumDeviceSettingsCombo* pBestDeviceSettingsCombo = NULL;
    DXGI_MODE_DESC adapterDisplayMode;
    ...
    for( int iAdapter = 0; iAdapter < pAdapterList->GetSize(); iAdapter++ )
    {
        ...
        for( int iDeviceCombo = 0; iDeviceCombo < pAdapterInfo->deviceSettingsComboList.GetSize(); iDeviceCombo++ )
        {
            ...
            // Skip any combo that doesn't meet the preserve match options
            if( false == DXUTDoesD3D11DeviceComboMatchPreserveOptions( pDeviceSettingsCombo, pIn, pMatchOptions ) )
                continue;

            // Get a ranking number that describes how closely this device combo matches the optimal combo
            float fCurRanking = DXUTRankD3D11DeviceCombo( pDeviceSettingsCombo, pOptimal, &adapterDisplayMode );
    ...
    // Using the best device settings combo found, build valid device settings taking heed of 
    // the match options and the input device settings
    DXUTD3D11DeviceSettings validDeviceSettings;
    DXUTBuildValidD3D11DeviceSettings( &validDeviceSettings, pBestDeviceSettingsCombo, pIn, pMatchOptions );
    *pOut = validDeviceSettings;
...
//--------------------------------------------------------------------------------------
// Internal helper function to build a D3D11 device settings structure based upon the match 
// options.  If the match option is set to ignore, then a optimal default value is used.
// The default value may not exist on the system, but later this will be taken 
// into account.
//--------------------------------------------------------------------------------------
void DXUTBuildOptimalD3D11DeviceSettings( DXUTD3D11DeviceSettings* pOptimalDeviceSettings,
                                          DXUTD3D11DeviceSettings* pDeviceSettingsIn,
                                          DXUTMatchOptions* pMatchOptions )
{
    ZeroMemory( pOptimalDeviceSettings, sizeof( DXUTD3D11DeviceSettings ) );
    ...
    DXUTGetD3D11AdapterDisplayMode( pOptimalDeviceSettings->AdapterOrdinal, 0, &adapterDesktopDisplayMode );
    ...
    //---------------------
    if( pMatchOptions->eDeviceType == DXUTMT_IGNORE_INPUT )
        pOptimalDeviceSettings->DriverType = D3D_DRIVER_TYPE_HARDWARE;
    else
        pOptimalDeviceSettings->DriverType = pDeviceSettingsIn->DriverType;
    ...
    {
        pOptimalDeviceSettings->AutoCreateDepthStencil = TRUE;
        pOptimalDeviceSettings->AutoDepthStencilFormat = DXGI_FORMAT_D24_UNORM_S8_UINT;
    }
    else
    {
        pOptimalDeviceSettings->AutoCreateDepthStencil = pDeviceSettingsIn->AutoCreateDepthStencil;
...
// match options against the input pDeviceSettingsIn.
//--------------------------------------------------------------------------------------
bool DXUTDoesD3D11DeviceComboMatchPreserveOptions( CD3D11EnumDeviceSettingsCombo* pDeviceSettingsCombo,
                                                   DXUTD3D11DeviceSettings* pDeviceSettingsIn,
                                                   DXUTMatchOptions* pMatchOptions )
{
    //---------------------
...
// best device settings combo found.
//--------------------------------------------------------------------------------------
void DXUTBuildValidD3D11DeviceSettings( DXUTD3D11DeviceSettings* pValidDeviceSettings,
                                        CD3D11EnumDeviceSettingsCombo* pBestDeviceSettingsCombo,
                                        DXUTD3D11DeviceSettings* pDeviceSettingsIn,
                                        DXUTMatchOptions* pMatchOptions )
{
    DXGI_MODE_DESC adapterDisplayMode;
    DXUTGetD3D11AdapterDisplayMode( pBestDeviceSettingsCombo->AdapterOrdinal,
                                    pBestDeviceSettingsCombo->Output, &adapterDisplayMode );
    ...
    else
    {
        ...
        // Call a helper function to find the closest valid display mode to the optimal
        DXUTFindValidD3D11Resolution( pBestDeviceSettingsCombo, displayModeIn, &bestDisplayMode );
    }
    ...
    {
        bestEnableAutoDepthStencil = true;
        bestDepthStencilFormat =  DXGI_FORMAT_D24_UNORM_S8_UINT;
    }
    else
    {
        bestEnableAutoDepthStencil = pDeviceSettingsIn->AutoCreateDepthStencil;
    ...
    else if( pMatchOptions->ePresentInterval == DXUTMT_IGNORE_INPUT )
    {
        ...
        // produce _com_error exceptions when the sync interval is
        // anything but 0.
        bestPresentInterval = ( D3D_DRIVER_TYPE_REFERENCE == pBestDeviceSettingsCombo->DeviceType ) ? 0 : 1;
    }
    else // if( pMatchOptions->ePresentInterval == DXUTMT_CLOSEST_TO_INPUT )
    {
        bestPresentInterval = pDeviceSettingsIn->SyncInterval;
    ...
    // Fill the device settings struct
    ZeroMemory( pValidDeviceSettings, sizeof(DXUTD3D11DeviceSettings) );
    pValidDeviceSettings->AdapterOrdinal                 = pBestDeviceSettingsCombo->AdapterOrdinal;
    pValidDeviceSettings->Output                         = pBestDeviceSettingsCombo->Output;
    pValidDeviceSettings->DriverType                     = pBestDeviceSettingsCombo->DeviceType;
    pValidDeviceSettings->sd.BufferDesc.Width            = bestDisplayMode.Width;
...
// Internal helper function to find the closest allowed display mode to the optimal 
//--------------------------------------------------------------------------------------
HRESULT DXUTFindValidD3D11Resolution( CD3D11EnumDeviceSettingsCombo* pBestDeviceSettingsCombo,
                                      DXGI_MODE_DESC displayModeIn, DXGI_MODE_DESC* pBestDisplayMode )
{
    DXGI_MODE_DESC bestDisplayMode;
    ZeroMemory( &bestDisplayMode, sizeof( bestDisplayMode ) );
